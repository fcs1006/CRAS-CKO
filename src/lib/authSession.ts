...
let runtimeGeneratedSecret: string | null = null

function getSecretKey(): string {
  const secret = process.env.AUTH_SECRET || process.env.SUPABASE_SERVICE_ROLE_KEY
  if (secret && secret.trim()) return secret.trim()

  // Se não foi configurado segredo explícito no ambiente (ex: painel Cloudflare ou .env),
  // gera um segredo criptográfico dinâmico por processo para que seja impossível forjar tokens externamente.
  if (!runtimeGeneratedSecret) {
    if (typeof crypto !== 'undefined' && crypto.getRandomValues) {
      const bytes = new Uint8Array(32)
      crypto.getRandomValues(bytes)
      runtimeGeneratedSecret = Array.from(bytes, b => b.toString(16).padStart(2, '0')).join('')
    } else {
      runtimeGeneratedSecret = Math.random().toString(36).substring(2) + Date.now().toString(36)
    }
  }
