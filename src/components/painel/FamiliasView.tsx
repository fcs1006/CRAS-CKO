...
export function FamiliasView({
  ...
  // Fatiamento dos registros conforme a página selecionada
  const inicioSlice = (paginaAtual - 1) * itensPorPagina
  const fimSlice = inicioSlice + itensPorPagina
  const familiasPaginadas = familiasFiltradas.slice(inicioSlice, fimSlice)
          ...
          <div className="flex items-center gap-3 shrink-0 flex-wrap">
            <div className="hidden lg:flex items-center gap-3 bg-white/5 backdrop-blur-md p-2.5 rounded-xl border border-white/10 text-xs">
              <div className="px-3 py-1 text-center">
                ...
                <strong className="text-base font-black text-white">{familias.length.toLocaleString('pt-BR')}</strong>
              </div>
            </div>
          ...
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-gray-100 text-xs">
              {familiasFiltradas.length === 0 ? (
                ...
                familiasPaginadas.map(({ familia: f, membroCorrespondente }) => {
