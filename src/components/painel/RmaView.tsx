...
export function RmaView({
  ...
  // Funções de navegação rápida de Mês / Ano
  const handleMesAnterior = () => {
    ...
    setMesAno(`${novoAno}-${String(novoMes).padStart(2, '0')}`)
  }
  ...
  const handleMudarMes = (m: string) => {
    setMesAno(`${anoRef}-${m}`)
  }

  const handleMudarAno = (a: string) => {
    setMesAno(`${a}-${mesRef}`)
  }

  const anosDisponiveis = useMemo(() => {
    const currentYear = new Date().getFullYear()
    const setAnos = new Set<number>([currentYear - 2, currentYear - 1, currentYear, currentYear + 1, currentYear + 2])
    const anoSelecionado = parseInt(anoRef, 10)
    if (!isNaN(anoSelecionado)) setAnos.add(anoSelecionado)
    return Array.from(setAnos).sort((a, b) => a - b)
  }, [anoRef])
  ...
  return (
    <div className="space-y-6">
      {/* ========================================================
          DOCUMENTO DE IMPRESSÃO OFICIAL DO RMA (PADRÃO MDS/SUAS)
          Utiliza DocumentoOficialLayout (padrão de todos os formulários)
          ======================================================== */}
      <div className="hidden print:block w-full">
        <DocumentoOficialLayout
          configuracao={configuracao}
          tituloDocumento="RMA OFICIAL"
          subtituloDocumento={`MÊS: ${mesExtenso.toUpperCase()} / ${anoRef}`}
          subirAssinaturas={true}
          assinaturas={
            <div className="grid grid-cols-2 gap-8 text-center text-[10px] pt-1 pb-0">
              <div className="border-t-[1.5px] border-black pt-1">
                <p className="font-extrabold uppercase text-[10.5px] text-black">
                  {usuarioLogado?.nome || 'TÉCNICO(A) DE REFERÊNCIA'}
                </p>
                <p className="text-[9px] font-semibold text-black mt-0.5">
                  {usuarioLogado?.cargo ? `${usuarioLogado.cargo}${usuarioLogado.conselho ? ` (${usuarioLogado.conselho})` : ''}` : 'Responsável Técnico(a) pelo Preenchimento do RMA'}
                </p>
              </div>
              <div className="border-t-[1.5px] border-black pt-1">
                <p className="font-extrabold uppercase text-[10.5px] text-black">
                ...
                <p className="text-[9px] font-semibold text-black mt-0.5">
                  Visto e Homologação da Coordenação do CRAS
                </p>
              </div>
            </div>
          }
        >
          <div className="space-y-2 text-black">
            ...
            <div className="border border-black">
              <div className="bg-black text-white px-2 py-1 font-black text-[10px] uppercase flex justify-between items-center">
                <span>BLOCO 1 — FAMÍLIAS EM ACOMPANHAMENTO PELO PAIF</span>
                <span className="font-mono text-[9px]">MÊS: {mesExtenso.toUpperCase()} / {anoRef}</span>
              </div>

              <div className="bg-gray-100 px-2 py-0.5 border-b border-black text-[9px] font-black uppercase text-black">
                A. Volume de famílias em acompanhamento pelo PAIF
              </div>
              <table className="w-full border-collapse text-[9.5px]">
                <tbody>
                  <tr className="border-b border-black">
                    <td className="w-12 text-center font-bold bg-gray-50 border-r border-black p-1">A.1</td>
                    <td className="p-1 border-r border-black">Total de famílias em acompanhamento pelo PAIF no mês de referência</td>
                    <td className="w-20 text-center font-black font-mono text-[11px] p-1">{rmaData.A1.total}</td>
                  </tr>
                  <tr className="border-b border-black">
                    <td className="w-12 text-center font-bold bg-gray-50 border-r border-black p-1">A.2</td>
                    <td className="p-1 border-r border-black">Novas famílias inseridas no acompanhamento do PAIF durante o mês de referência</td>
                    <td className="w-20 text-center font-black font-mono text-[11px] p-1">{rmaData.A2.total}</td>
                  </tr>
                </tbody>
              </table>

              <div className="bg-gray-100 px-2 py-0.5 border-t border-b border-black text-[9px] font-black uppercase text-black">
                B. Perfil das novas famílias inseridas em acompanhamento no PAIF no mês de referência
              </div>
              <table className="w-full border-collapse text-[9.5px]">
                <tbody>
                  <tr className="border-b border-black">
                    <td className="w-12 text-center font-bold bg-gray-50 border-r border-black p-1">B.1</td>
                    <td className="p-1 border-r border-black">Famílias em situação de extrema pobreza (renda per capita mensal de até R$ 218,00)</td>
                    <td className="w-20 text-center font-black font-mono text-[11px] p-1">{rmaData.B1.total}</td>
                  </tr>
                  <tr className="border-b border-black">
                    <td className="w-12 text-center font-bold bg-gray-50 border-r border-black p-1">B.2</td>
                    <td className="p-1 border-r border-black">Famílias beneficiárias do Programa Bolsa Família (PBF)</td>
                    <td className="w-20 text-center font-black font-mono text-[11px] p-1">{rmaData.B2.total}</td>
                  </tr>
                  <tr className="border-b border-black">
                    <td className="w-12 text-center font-bold bg-gray-50 border-r border-black p-1">B.3</td>
                    <td className="p-1 border-r border-black">Famílias beneficiárias do Bolsa Família em descumprimento de condicionalidades</td>
                    <td className="w-20 text-center font-black font-mono text-[11px] p-1">{rmaData.B3.total}</td>
                  </tr>
                  <tr className="border-b border-black">
                    <td className="w-12 text-center font-bold bg-gray-50 border-r border-black p-1">B.4</td>
                    <td className="p-1 border-r border-black">Famílias com membros beneficiários do Benefício de Prestação Continuada (BPC)</td>
                    <td className="w-20 text-center font-black font-mono text-[11px] p-1">{rmaData.B4.total}</td>
                  </tr>
                  <tr className="border-b border-black">
                    <td className="w-12 text-center font-bold bg-gray-50 border-r border-black p-1">B.5</td>
                    <td className="p-1 border-r border-black">Famílias com crianças ou adolescentes em situação de trabalho infantil</td>
                    <td className="w-20 text-center font-black font-mono text-[11px] p-1">{rmaData.B5.total}</td>
                  </tr>
                  <tr>
                    <td className="w-12 text-center font-bold bg-gray-50 border-r border-black p-1">B.6</td>
                    <td className="p-1 border-r border-black">Famílias com crianças ou adolescentes em Serviço de Acolhimento Institucional</td>
                    <td className="w-20 text-center font-black font-mono text-[11px] p-1">{rmaData.B6.total}</td>
                  </tr>
                </tbody>
              </table>
            </div>
            ...
            <div className="border border-black">
              <div className="bg-black text-white px-2 py-1 font-black text-[10px] uppercase flex justify-between items-center">
                <span>BLOCO 2 — ATENDIMENTOS PARTICULARIZADOS REALIZADOS NO CRAS</span>
                <span className="font-mono text-[9px]">MÊS: {mesExtenso.toUpperCase()} / {anoRef}</span>
              </div>
              ...
              <table className="w-full border-collapse text-[9.5px]">
                <tbody>
                  <tr className="border-b border-black">
                    <td className="w-12 text-center font-bold bg-gray-50 border-r border-black p-1">C.1</td>
                    <td className="p-1 border-r border-black">Total de atendimentos particularizados realizados no mês de referência</td>
                    <td className="w-20 text-center font-black font-mono text-[11px] p-1">{rmaData.C1.total}</td>
                  </tr>
                  <tr className="border-b border-black">
                    <td className="w-12 text-center font-bold bg-gray-50 border-r border-black p-1">C.2</td>
                    <td className="p-1 border-r border-black">Famílias encaminhadas para inclusão no Cadastro Único (CadÚnico)</td>
                    <td className="w-20 text-center font-black font-mono text-[11px] p-1">{rmaData.C2.total}</td>
                  </tr>
                  <tr className="border-b border-black">
                    <td className="w-12 text-center font-bold bg-gray-50 border-r border-black p-1">C.3</td>
                    <td className="p-1 border-r border-black">Famílias encaminhadas para atualização cadastral no Cadastro Único (CadÚnico)</td>
                    <td className="w-20 text-center font-black font-mono text-[11px] p-1">{rmaData.C3.total}</td>
                  </tr>
                  <tr className="border-b border-black">
                    <td className="w-12 text-center font-bold bg-gray-50 border-r border-black p-1">C.4</td>
                    <td className="p-1 border-r border-black">Indivíduos encaminhados para acesso ao Benefício de Prestação Continuada (BPC)</td>
                    <td className="w-20 text-center font-black font-mono text-[11px] p-1">{rmaData.C4.total}</td>
                  </tr>
                  <tr className="border-b border-black">
                    <td className="w-12 text-center font-bold bg-gray-50 border-r border-black p-1">C.5</td>
                    <td className="p-1 border-r border-black">Famílias encaminhadas para o CREAS / PAEFI</td>
                    <td className="w-20 text-center font-black font-mono text-[11px] p-1">{rmaData.C5.total}</td>
                  </tr>
                  <tr className="border-b border-black">
                    <td className="w-12 text-center font-bold bg-gray-50 border-r border-black p-1">C.6</td>
                    <td className="p-1 border-r border-black">Visitas domiciliares realizadas no mês de referência</td>
                    <td className="w-20 text-center font-black font-mono text-[11px] p-1">{rmaData.C6.total}</td>
                  </tr>
                  <tr className="border-b border-black">
                    <td className="w-12 text-center font-bold bg-gray-50 border-r border-black p-1">C.7</td>
                    <td className="p-1 border-r border-black">Total de auxílios-natalidade concedidos / entregues no mês de referência</td>
                    <td className="w-20 text-center font-black font-mono text-[11px] p-1">{rmaData.C7.total}</td>
                  </tr>
                  <tr className="border-b border-black">
                    <td className="w-12 text-center font-bold bg-gray-50 border-r border-black p-1">C.8</td>
                    <td className="p-1 border-r border-black">Total de auxílios-funeral concedidos / entregues no mês de referência</td>
                    <td className="w-20 text-center font-black font-mono text-[11px] p-1">{rmaData.C8.total}</td>
                  </tr>
                  <tr>
                    <td className="w-12 text-center font-bold bg-gray-50 border-r border-black p-1">C.9</td>
                    <td className="p-1 border-r border-black">Outros benefícios eventuais concedidos / entregues no mês</td>
                    <td className="w-20 text-center font-black font-mono text-[11px] p-1">{rmaData.C9.total}</td>
                  </tr>
                </tbody>
              </table>
            </div>
            ...
            <div className="border border-black">
              <div className="bg-black text-white px-2 py-1 font-black text-[10px] uppercase flex justify-between items-center">
                <span>BLOCO 3 — ATENDIMENTOS COLETIVOS REALIZADOS NO CRAS</span>
                <span className="font-mono text-[9px]">MÊS: {mesExtenso.toUpperCase()} / {anoRef}</span>
              </div>

              <div className="bg-gray-100 px-2 py-0.5 border-b border-black text-[9px] font-black uppercase text-black">
                D. Volume de atendimentos coletivos realizados no CRAS no mês de referência
              </div>
              <table className="w-full border-collapse text-[9.5px]">
                <tbody>
                  <tr className="border-b border-black">
                    <td className="w-12 text-center font-bold bg-gray-50 border-r border-black p-1">D.1</td>
                    <td className="p-1 border-r border-black">Famílias participando regularmente de grupos do PAIF (oficinas continuadas)</td>
                    <td className="w-20 text-center font-black font-mono text-[11px] p-1">{rmaData.D1.total}</td>
                  </tr>
                  <tr className="border-b border-black">
                    <td className="w-12 text-center font-bold bg-gray-50 border-r border-black p-1">D.2</td>
                    <td className="p-1 border-r border-black">Crianças de 0 a 6 anos participando do SCFV</td>
                    <td className="w-20 text-center font-black font-mono text-[11px] p-1">{rmaData.D2.total}</td>
                  </tr>
                  <tr className="border-b border-black">
                    <td className="w-12 text-center font-bold bg-gray-50 border-r border-black p-1">D.3</td>
                    <td className="p-1 border-r border-black">Crianças e adolescentes de 7 a 14 anos participando do SCFV</td>
                    <td className="w-20 text-center font-black font-mono text-[11px] p-1">{rmaData.D3.total}</td>
                  </tr>
                  <tr className="border-b border-black">
                    <td className="w-12 text-center font-bold bg-gray-50 border-r border-black p-1">D.4</td>
                    <td className="p-1 border-r border-black">Adolescentes de 15 a 17 anos participando do SCFV</td>
                    <td className="w-20 text-center font-black font-mono text-[11px] p-1">{rmaData.D4.total}</td>
                  </tr>
                  <tr className="border-b border-black">
                    <td className="w-12 text-center font-bold bg-gray-50 border-r border-black p-1">D.8</td>
                    <td className="p-1 border-r border-black">Adultos de 18 a 59 anos participando de grupos do SCFV</td>
                    <td className="w-20 text-center font-black font-mono text-[11px] p-1">{rmaData.D8.total}</td>
                  </tr>
                  <tr className="border-b border-black">
                    <td className="w-12 text-center font-bold bg-gray-50 border-r border-black p-1">D.5</td>
                    <td className="p-1 border-r border-black">Pessoas idosas (60 anos ou mais) participando de grupos do SCFV para Idosos</td>
                    <td className="w-20 text-center font-black font-mono text-[11px] p-1">{rmaData.D5.total}</td>
                  </tr>
                  <tr className="border-b border-black">
                    <td className="w-12 text-center font-bold bg-gray-50 border-r border-black p-1">D.6</td>
                    <td className="p-1 border-r border-black">Participantes de palestras, oficinas e atividades coletivas de caráter não continuado</td>
                    <td className="w-20 text-center font-black font-mono text-[11px] p-1">{rmaData.D6.total}</td>
                  </tr>
                  <tr>
                    <td className="w-12 text-center font-bold bg-gray-50 border-r border-black p-1">D.7</td>
                    <td className="p-1 border-r border-black">Pessoas com deficiência (PcD) participando do SCFV ou de grupos continuados do PAIF</td>
                    <td className="w-20 text-center font-black font-mono text-[11px] p-1">{rmaData.D7.total}</td>
                  </tr>
                </tbody>
              </table>
            </div>
      ...
      <div className="bg-gradient-to-r from-slate-900 via-teal-950 to-teal-900 rounded-2xl p-6 text-white shadow-xl border border-teal-800/40 relative overflow-hidden print:hidden">
        ...
        <div className="relative z-10 flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
          ...
          <div className="flex items-center gap-3 shrink-0 flex-wrap">
            {/* Seletor Visual Elegante de Mês e Ano */}
            <div className="flex items-center gap-1.5 bg-black/40 backdrop-blur-md p-1.5 rounded-xl border border-teal-500/30 shadow-inner">
              <button
                type="button"
                onClick={handleMesAnterior}
                title="Mês anterior"
                className="h-8 w-8 flex items-center justify-center rounded-lg bg-white/10 hover:bg-teal-500 hover:text-slate-950 text-teal-200 transition text-xs font-bold"
              >
              ...
              <div className="flex items-center gap-1">
                <div className="relative">
                  <select
                    ...
                    className="h-8 pl-2.5 pr-7 bg-slate-900/95 text-white font-bold text-xs rounded-lg border border-teal-500/40 focus:outline-none focus:ring-2 focus:ring-teal-400 appearance-none cursor-pointer"
                  >
                    {mesesNomes.map((nome, idx) => {
                      const val = String(idx + 1).padStart(2, '0')
                      return (
                        <option key={val} value={val} className="bg-slate-900 text-white font-medium">
                          {nome}
                        </option>
                      )
                    })}
                  </select>
                  <i className="fa-solid fa-chevron-down absolute right-2 top-2.5 text-[10px] text-teal-400 pointer-events-none"></i>
                </div>
                ...
                <div className="relative">
                  <select
                    ...
                    className="h-8 pl-2.5 pr-6 bg-slate-900/95 text-white font-bold text-xs rounded-lg border border-teal-500/40 focus:outline-none focus:ring-2 focus:ring-teal-400 appearance-none cursor-pointer font-mono"
                  >
                    {anosDisponiveis.map(a => (
                      <option key={a} value={String(a)} className="bg-slate-900 text-white font-medium font-mono">
                        {a}
                      </option>
                    ))}
                  </select>
              ...
              {mesAno !== currentMonthStr && (
                <button
                  type="button"
                  onClick={() => setMesAno(currentMonthStr)}
                  title="Voltar ao mês atual"
                  className="h-8 px-2.5 rounded-lg bg-teal-500/20 hover:bg-teal-500 hover:text-slate-950 text-teal-300 text-[11px] font-extrabold uppercase transition border border-teal-400/30 flex items-center gap-1"
                >
            ...
            <button
              ...
              className="bg-emerald-500 hover:bg-emerald-400 text-slate-950 font-black px-4 py-2 rounded-xl text-xs shadow-lg transition flex items-center gap-2 hover:scale-[1.02] active:scale-95 border border-emerald-300"
            >
      ...
      {/* ========================================================
          CONTEÚDO INTERATIVO EM TELA (BLOCOS 1, 2 E 3)
          ======================================================== */}
      <div className="space-y-6 print:hidden">
        {/* BLOCO 1 - FAMÍLIAS EM ACOMPANHAMENTO PELO PAIF */}
        <div className="bg-white rounded-2xl border border-teal-900/20 shadow-sm overflow-hidden">
