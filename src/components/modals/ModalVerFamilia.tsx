            ...
            <div className="overflow-x-auto border border-gray-200 rounded-xl">
              <table className="w-full text-left divide-y divide-gray-200 text-xs">
                <thead className="bg-gray-50 text-[10px] font-bold text-gray-700 uppercase tracking-wider">
                  <tr>
                    <th className="py-2.5 px-3 min-w-[180px]">Nome do Integrante</th>
                    <th className="py-2.5 px-2.5 w-28 whitespace-nowrap">Parentesco</th>
                    <th className="py-2.5 px-2.5 w-20 whitespace-nowrap text-left">Idade</th>
                    <th className="py-2.5 px-2.5 w-32 whitespace-nowrap text-left">CPF / NIS</th>
                    <th className="py-2.5 px-2.5 max-w-[190px] text-left">Ocupação / CBO</th>
                    <th className="py-2.5 px-2.5 w-24 whitespace-nowrap text-left">Prog. Social</th>
                    <th className="py-2.5 px-3 w-28 whitespace-nowrap text-left">Renda Indiv.</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-100 text-xs">
                  ...
                  <tr className="bg-teal-50/40 font-semibold">
                    ...
                    <td className="py-2.5 px-2.5 whitespace-nowrap">
                      <span className="px-2 py-0.5 bg-teal-200 text-teal-900 rounded font-bold text-[10px] uppercase">Responsável</span>
                    </td>
                    <td className="py-2.5 px-2.5 whitespace-nowrap text-left">{calculateAge(familia.data_nascimento_responsavel || membroResp?.data_nascimento || '')} anos</td>
                    <td className="py-2.5 px-2.5 font-mono text-[11px] whitespace-nowrap text-left">{familia.cpf_responsavel ? maskCPF(familia.cpf_responsavel) : '—'}</td>
                    <td className="py-2.5 px-2.5 uppercase max-w-[190px] truncate text-[11px] text-left" title={familia.ocupacao_responsavel || membroResp?.ocupacao || '—'}>
                      {familia.ocupacao_responsavel || membroResp?.ocupacao || '—'}
                    </td>
                    <td className="py-2.5 px-2.5 uppercase whitespace-nowrap text-left">{familia.programa_social_responsavel || 'Nenhum'}</td>
                    <td className="py-2.5 px-3 text-left font-mono font-bold text-emerald-800 whitespace-nowrap">R$ {rendaResp.toFixed(2)}</td>
                  </tr>
                  ...
                  {outrosMembros.map((m, idx) => (
                    <tr key={idx} className="hover:bg-gray-50">
                      ...
                      <td className="py-2.5 px-2.5 uppercase text-gray-600 whitespace-nowrap">{m.parentesco}</td>
                      <td className="py-2.5 px-2.5 text-gray-600 whitespace-nowrap text-left">{m.idade} anos</td>
                      <td className="py-2.5 px-2.5 font-mono text-[11px] text-gray-600 whitespace-nowrap text-left">{m.cpf ? maskCPF(m.cpf) : (m.nis ? maskNIS(m.nis) : '—')}</td>
                      <td className="py-2.5 px-2.5 uppercase text-gray-600 max-w-[190px] truncate text-[11px] text-left" title={m.ocupacao || '—'}>
                        {m.ocupacao || '—'}
                      </td>
                      <td className="py-2.5 px-2.5 uppercase text-gray-600 whitespace-nowrap text-left">{m.programa_governo || 'Nenhum'}</td>
                      <td className="py-2.5 px-3 text-left font-mono font-semibold text-gray-900 whitespace-nowrap">R$ {(m.renda || 0).toFixed(2)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
