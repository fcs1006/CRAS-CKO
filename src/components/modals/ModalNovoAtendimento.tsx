...
export function ModalNovoAtendimento({
  ...
  function selecionarFamilia(f: Familia) {
    ...
    // Se o tipo atual for exclusivo do PAIF e a família não tiver PAIF ativo, redefinir para Atendimento Particularizado
    if ((tipo === 'Acompanhamento PAIF' || tipo === 'Visita Domiciliar') && !f.paif_ativo) {
      setTipo('Atendimento Particularizado')
      alert(`Atenção: A família selecionada (${f.responsavel}) não possui Acompanhamento PAIF ativo. A tipologia do atendimento foi ajustada para "Atendimento Particularizado", pois "${tipo}" é exclusivo para famílias com PAIF ativo.`)
    }
  }

  function handleTipoChange(novoTipo: string) {
    if ((novoTipo === 'Acompanhamento PAIF' || novoTipo === 'Visita Domiciliar') && familiaSelecionada && !familiaSelecionada.paif_ativo) {
      alert(`Bloqueio de Segurança SUAS:\nA família selecionada (${familiaSelecionada.responsavel}) não possui Acompanhamento PAIF ativo no cadastro.\n\nNão é permitido registrar "${novoTipo}" para famílias fora do PAIF. Selecione "Atendimento Particularizado" ou "Recepção / Acolhida Inicial", ou ative o PAIF no Prontuário da família.`)
      setTipo('Atendimento Particularizado')
      return
    }
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    // BLOQUEIO RESTRITO PAIF: Não permitir salvar Acompanhamento PAIF ou Visita Domiciliar para famílias sem PAIF ativo
    const ehTipoExclusivoPaif = tipo === 'Acompanhamento PAIF' || tipo === 'Visita Domiciliar'
    if (ehTipoExclusivoPaif && (!familiaSelecionada || !familiaSelecionada.paif_ativo)) {
      alert(`BLOQUEIO DE SEGURANÇA SUAS:\nA família "${familiaSelecionada?.responsavel || 'selecionada'}" NÃO possui Acompanhamento PAIF ativo no cadastro.\n\nO sistema não permite registrar "${tipo}" para famílias fora do PAIF.\n\nPor favor, selecione "Atendimento Particularizado" (demanda pontual) ou "Recepção / Acolhida Inicial", ou ative o Acompanhamento PAIF no Prontuário da família antes de salvar.`)
      return
    }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4 overflow-y-auto">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-2xl my-8 overflow-hidden flex flex-col max-h-[90vh]">
        ...
        <form onSubmit={handleSubmit} className="p-6 overflow-y-auto space-y-4 flex-1 text-xs">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5">
            ...
            <div>
              ...
              <select
                ...
                required
                className={`w-full px-3 py-2 border rounded-lg text-xs bg-white font-bold uppercase truncate transition ${
                  (tipo === 'Acompanhamento PAIF' || tipo === 'Visita Domiciliar') && familiaSelecionada && !familiaSelecionada.paif_ativo
                    ? 'border-red-500 text-red-700 bg-red-50 ring-2 ring-red-200'
                    : 'text-gray-900'
                }`}
              >
                ...
                <option
                  ...
                  disabled={Boolean(familiaSelecionada && !familiaSelecionada.paif_ativo)}
                >
                  ACOMPANHAMENTO PAIF {familiaSelecionada && !familiaSelecionada.paif_ativo ? '(BLOQUEADO: EXIGE PAIF ATIVO)' : ''}
                </option>
                <option
                  value="Visita Domiciliar"
                  disabled={Boolean(familiaSelecionada && !familiaSelecionada.paif_ativo)}
                >
                  VISITA DOMICILIAR (PAIF / TERRITÓRIO) {familiaSelecionada && !familiaSelecionada.paif_ativo ? '(BLOQUEADO: EXIGE PAIF ATIVO)' : ''}
                </option>
          ...
          {/* Verificação e Alerta de Bloqueio se Família não possuir PAIF Ativo */}
          {(tipo === 'Acompanhamento PAIF' || tipo === 'Visita Domiciliar') && familiaSelecionada && !familiaSelecionada.paif_ativo && (
            <div className="p-3.5 bg-red-50 border-2 border-red-300 rounded-xl text-red-950 text-xs flex items-start gap-2.5 shadow-sm">
              <i className="fa-solid fa-circle-exclamation text-red-600 mt-0.5 text-base shrink-0"></i>
              <div className="space-y-1">
                <strong className="block uppercase font-bold text-red-950">
                  Bloqueio Normativo: Esta família não possui Acompanhamento PAIF ativo no cadastro.
                </strong>
                <p className="leading-relaxed text-[11px] text-red-800">
                  O sistema <strong>não permite salvar</strong> atendimentos de <strong>"{tipo}"</strong> para famílias fora do PAIF.
                </p>
                <p className="leading-relaxed text-[11px] text-red-700 font-medium">
                  Para registrar este atendimento pontual, selecione <strong>"Atendimento Particularizado"</strong> ou <strong>"Recepção / Acolhida Inicial"</strong>. Caso a família deva ingressar no acompanhamento familiar continuado, ative o PAIF no Prontuário da família.
                </p>
              </div>
            </div>
          )}
          ...
          <div className="pt-3 border-t flex justify-end gap-2 shrink-0">
            ...
            {(() => {
              const ehBloqueado = (tipo === 'Acompanhamento PAIF' || tipo === 'Visita Domiciliar') && Boolean(familiaSelecionada && !familiaSelecionada.paif_ativo)
              return (
                <button
                  type="submit"
                  disabled={salvando || ehBloqueado}
                  title={ehBloqueado ? 'Selecione "Atendimento Particularizado" ou ative o PAIF no Prontuário para salvar.' : undefined}
                  className={`px-6 py-2 rounded-xl text-xs font-bold shadow-md transition uppercase flex items-center gap-2 ${
                    ehBloqueado
                      ? 'bg-gray-300 text-gray-600 cursor-not-allowed border border-gray-400 opacity-80'
                      : 'bg-teal-800 hover:bg-teal-900 text-white'
                  }`}
                >
                  <i className={`fa-solid ${salvando ? 'fa-spinner fa-spin' : ehBloqueado ? 'fa-lock text-red-700' : 'fa-floppy-disk'}`}></i>
                  {salvando ? 'Salvando...' : ehBloqueado ? 'Bloqueado (Exige PAIF Ativo)' : 'Salvar Atendimento'}
                </button>
              )
            })()}
          </div>
