...
interface ModalNovoProfissionalProps {
  onFechar: () => void
  onSalvar: (dados: {
    nome: string
    cpf: string
...
export function ModalNovoProfissional({ onFechar, onSalvar }: ModalNovoProfissionalProps) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!senha.trim() || senha.length < 4) {
      setErro('A senha inicial deve ter pelo menos 4 caracteres.')
      return
    }
    ...
    try {
      ...
      setErro(err.message || 'Erro ao cadastrar profissional.')
    } finally {
      setSalvando(false)
    }
  }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-3 sm:p-4 overflow-y-auto">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-xl my-6 overflow-hidden flex flex-col animate-in fade-in zoom-in-95 duration-200">
        ...
        <div className="bg-slate-800 text-white p-5 flex justify-between items-center shrink-0">
          <div>
            <h3 className="text-base font-bold flex items-center gap-2 uppercase tracking-wide">
              <i className="fa-solid fa-user-shield text-emerald-400"></i> Cadastrar Novo Profissional / Técnico
            </h3>
            <p className="text-xs text-slate-300 mt-0.5">
              Habilitação de acesso ao Sistema SUAS Digital do CRAS
            </p>
          </div>
          <button
            type="button"
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4 text-xs overflow-y-auto max-h-[80vh]">
          ...
          {/* CPF e Perfil de Acesso */}
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div>
              <label className="block text-xs font-semibold text-gray-700 mb-1">
                CPF do Profissional <span className="text-red-600 font-bold">*</span>
              </label>
              <input
                type="text"
                required
                ...
                className="w-full px-3 py-2 border rounded-lg text-xs font-mono font-semibold text-gray-900 focus:ring-2 focus:ring-slate-500/20 focus:border-slate-800"
              />
              <span className="text-[10px] text-gray-400">O CPF será o login de acesso do profissional.</span>
            </div>
            ...
            <div>
              ...
              <select
                ...
                className="w-full px-3 py-2 border rounded-lg text-xs font-semibold text-gray-900 bg-white focus:ring-2 focus:ring-slate-500/20 focus:border-slate-800"
              >
                <option value="usuario">TÉCNICO / OPERADOR (Padrão)</option>
                <option value="admin">ADMINISTRADOR DO SISTEMA</option>
              </select>
              <span className="text-[10px] text-gray-400">Administradores podem gerenciar usuários e configurações.</span>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div>
              ...
              <select
                ...
                className="w-full px-3 py-2 border rounded-lg text-xs font-semibold text-gray-900 bg-white focus:ring-2 focus:ring-slate-500/20 focus:border-slate-800 uppercase"
              >
          ...
          <div className="p-4 bg-slate-50 border border-slate-200 rounded-xl space-y-2">
            <div className="flex justify-between items-center">
              <label className="block text-xs font-bold text-slate-800">
                Senha Inicial de Acesso <span className="text-red-600 font-bold">*</span>
              </label>
              <button
                type="button"
              ...
              >
                <i className="fa-solid fa-key"></i> Gerar Senha Aleatória
              </button>
            </div>
            ...
            <p className="text-[10px] text-slate-500">
              O profissional poderá alterar a senha a qualquer momento no perfil dele.
            </p>
          </div>
          ...
          <div className="pt-3 border-t flex justify-end gap-2">
            ...
            >
              {salvando ? (
                <>
                  <i className="fa-solid fa-circle-notch animate-spin"></i> Cadastrando...
                </>
              ) : (
                <>
