...
export default function PainelPage() {
  ...
  const [totalFamiliasContagem, setTotalFamiliasContagem] = useState<number | undefined>(undefined)
  const [totalPaifContagem, setTotalPaifContagem] = useState<number | undefined>(undefined)
  ...
  async function carregarTodosOsDados() {
    try {
      ...
      if (famRes.ok && famRes.headers.get('content-type')?.includes('application/json')) {
        ...
        if (famJson.ok && famJson.data) {
          setFamilias(famJson.data as Familia[])
          if (typeof famJson.total === 'number') {
            setTotalFamiliasContagem(famJson.total)
          }
        }
      }
  ...
  async function handleTogglePaif(familiaId: string, paifAtual: boolean) {
    ...
    try {
      await fetch('/api/familias', {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ id: familiaId, paif_ativo: !paifAtual })
      })
    } catch (err) {
  ...
  async function handleSalvarRelatorioGrupo(dados: {
    ...
    try {
      await fetch('/api/atendimentos', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(registrosHistorico)
      })
    } catch (e) {
  ...
  return (
    <div className="flex h-screen w-screen overflow-hidden bg-slate-50 font-sans print:h-auto print:w-full print:bg-white print:overflow-visible print:block">
      ...
      <aside className={`fixed lg:static z-40 h-full w-72 bg-gradient-to-b from-teal-900 to-teal-800 text-white flex flex-col justify-between p-5 shadow-xl transition-transform duration-300 print:hidden ${
        ...
        <div>
          {/* Logo / Header Sidebar */}
          <div className="flex items-center gap-3.5 pb-5 mb-5 border-b border-teal-700/60">
            {configuracao.logo_url ? (
              <img src={configuracao.logo_url} alt="Logo CRAS" className="h-14 w-auto max-w-[64px] object-contain rounded-xl bg-white/10 p-1 shrink-0 shadow-xs" />
            ) : (
              <div className="w-13 h-13 rounded-xl bg-teal-600 text-white flex items-center justify-center text-2xl font-bold shadow shrink-0">
                <i className="fa-solid fa-people-roof"></i>
              </div>
            )}
            <div className="min-w-0 flex-1">
              <h1 className="font-bold text-xs text-white leading-snug font-montserrat tracking-normal">
                <span className="block text-teal-100/90 text-[11px] font-medium leading-tight">Centro de Referência e</span>
                <span className="block text-teal-100/90 text-[11px] font-medium leading-tight">Assistência Social</span>
                <span className="block text-white font-extrabold text-xs leading-tight mt-0.5">Pedro de Santana Brito</span>
              </h1>
              <p className="text-[10px] text-teal-200 mt-1 leading-tight">
          ...
          {carregandoDados ? (
            ...
            <>
              {activeTab === 'dashboard' && (
                <DashboardView
                  familias={familias}
                  totalFamiliasContagem={totalFamiliasContagem}
                  totalPaifContagem={totalPaifContagem}
