...
-- ============================================================
-- ATIVAR ROW LEVEL SECURITY (RLS) E POLÍTICAS DE PROTEÇÃO
-- Todas as consultas do sistema operam via Next.js Server API
-- utilizando o service_role. O acesso direto do cliente (anon)
-- é bloqueado para impedir vazamento de prontuários e dados do SUAS.
-- ============================================================
...
ALTER TABLE beneficios_concedidos ENABLE ROW LEVEL SECURITY;
ALTER TABLE almoxarifado ENABLE ROW LEVEL SECURITY;
ALTER TABLE grupos_scfv ENABLE ROW LEVEL SECURITY;
ALTER TABLE participantes_scfv ENABLE ROW LEVEL SECURITY;
ALTER TABLE frequencia_scfv ENABLE ROW LEVEL SECURITY;
ALTER TABLE encaminhamentos ENABLE ROW LEVEL SECURITY;
ALTER TABLE agenda_tecnica ENABLE ROW LEVEL SECURITY;
ALTER TABLE pacientes ENABLE ROW LEVEL SECURITY;

-- Revogar acesso público direto via cliente anônimo a tabelas sensíveis
REVOKE ALL ON TABLE usuarios, familias, membros_familia, historico_atendimentos, beneficios_concedidos, almoxarifado, grupos_scfv, participantes_scfv, frequencia_scfv, encaminhamentos, agenda_tecnica, pacientes FROM anon;

-- Permitir apenas leitura das configurações públicas institucionais (Nome do CRAS, brasão)
GRANT SELECT ON TABLE configuracoes TO anon;

-- Conceder execução segura das RPCs necessárias para o fluxo de autenticação
